...
async def websocket_endpoint(websocket: WebSocket):
    ...
    # Try to create AI agent threads with error handling
    try:
        logger.info("Attempting to create AI agent threads...")
        thread = project_client.agents.threads.create()
        customer_loyalty_thread = project_client.agents.threads.create()
        logger.info("Successfully created AI agent threads")
        ai_agents_available = True
    except Exception as e:
        logger.error(f"Failed to create AI agent threads: {e}")
        logger.info("Continuing with fallback chat functionality...")
        # Create mock threads or use None to indicate unavailable
        thread = None
    ...
    async def run_customer_loyalty_task(customer_id):
        ...
        with tracer.start_as_current_span("Run Customer Loyalty Thread"):
            ...
            if not ai_agents_available or customer_loyalty_thread is None:
                session_loyalty_response = {"answer": "Customer loyalty calculation unavailable - AI agents not ready.", "agent": "customer_loyalty"}
                log_timing("Customer Loyalty Task", start_time, "AI agents unavailable")
                return
    ...
    welcome_message = "Welcome to Cora, your AI shopping assistant! I'm ready to help you with product recommendations, image analysis, and shopping questions."
    if not ai_agents_available:
        welcome_message += " (Note: Currently running in basic mode - advanced AI features may be limited)"
            ...
            try:
                ...
                if agent_name == "interior_designer":
                    ...
                    with tracer.start_as_current_span("Zava Interior Designer Agent Call"):
                        ...
                        if not image_url and not video_url:
                            ...
                            if image_url:
                                ...
                                # Placeholder for video analysis - not implemented yet
                                video_summary = "Video content analysis is not yet available. Please describe your space or upload an image instead."
                                ...
                                analysis_msg = "Video analysis is not yet supported. Please use image upload or describe your space."
                                await websocket.send_text(fast_json_dumps({"answer": analysis_msg, "agent": "interior_designer", "cart": persistent_cart}))
                    ...
                    with tracer.start_as_current_span("Zava Interior Designer Agent Call"):
                        ...
                        try:
                            if not persistent_image_url or not persistent_image_url.strip():
                                raise ValueError("No image URL available for image creation")
                            
                            # Validate that it's a proper image URL (not a search result or redirect)
                            if "bing.com" in persistent_image_url or "search" in persistent_image_url.lower():
                                raise ValueError("Search result URLs are not supported. Please provide a direct image URL.")
                            
                            image = create_image(text=user_message, image_url=persistent_image_url)
                            
                            if not image:
                                raise ValueError("Image creation failed - no image returned")
                            
                            # Create the response in the specified format and send directly to frontend
                            response_data = {
                                "answer": "Here is your requested image design! I've created a visualization based on your space and preferences.",
                                "products": "",
                            ...
                            # Fallback response when image creation fails
                            response_data = {
                                "answer": "I'm sorry, I couldn't create an image with the provided URL. This might be because the image URL is not directly accessible or is a search result page. Please try uploading a direct image URL or describe your space and I'll provide color and design recommendations instead.",
                                "products": "",
                                "discount_percentage": session_discount_percentage or "",
                                "image_url": "",
                                "video_url": "",
                                "additional_data": f"Error: {str(e)}",
                                "cart": persistent_cart
                            }
                    ...
                    # Check if AI agents are available
                    if not ai_agents_available or thread is None:
                        logger.debug(f"{agent_name} agent requested but AI agents unavailable - falling back to Cora")
                        # Fallback to Cora for any other agent type when AI agents are unavailable
                        prompt_for_cora = CORA_FALLBACK_PROMPT + formatted_history 
                        ...
                        log_timing("Cora Agent Call (Fallback)", cora_start_time, f"For {agent_name}")
                        msg = cora_fallback_reply
                        bot_reply = extract_bot_reply(msg)
                    else:
